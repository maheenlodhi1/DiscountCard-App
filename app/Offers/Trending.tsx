...
export default function HomeScreen() {
  const offers = useSelector((state: any) => state.auth.trendingPromotions);
  ...
  useFocusEffect(
    useCallback(() => {
      dispatch(getTrendingPromotions("top_n=100"));
    }, [dispatch])
  );

  const filteredOffers = offers?.trending_deals?.filter((offer: any) => {
            ...
            {categories?.results?.map((category: any, index: any) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  getOfferByCategory(
                    `top_n=100&categoryName=${category?.locale?.en?.title}`
                  );
                }}
              >
